	...
	double[][] notasCuantitativas = {{7.1, 8, 10}, {3, 7, 9}, {10, 9, 2.1}};
        ...
        for (int fila = 0; fila < notasCuantitativas.length ; fila++) {
            for (int col = 0; col < notasCuantitativas[fila].length; col++) {
                // < >
                if (notasCuantitativas[fila][col] >= 0.0 && notasCuantitativas[fila][col] <= 2.9){
                    notasCualitativas[fila][col]="Insuficiente";
                } else {
                    if (notasCuantitativas[fila][col] >= 3.0 && notasCuantitativas[fila][col] < 4.9 ) {
                    notasCualitativas[fila][col]="Regular";
                        
                    } else {
                        if (notasCuantitativas[fila][col] >= 5.0 && notasCuantitativas[fila][col] < 7.9   ) {
                    notasCualitativas[fila][col]="Buena";
                            
                        } else {
                            if (notasCuantitativas[fila][col] >= 8.0 & notasCuantitativas[fila][col] <= 9.5){
                    notasCualitativas[fila][col]="Muy Buena";
                                
                            } else {
                                if (notasCuantitativas[fila][col] >= 9.6 & notasCuantitativas[fila][col] <= 10){
